...
LOG_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/deskflow-watchdog"
mkdir -p "$LOG_DIR"
...
PREVENTIVE_RELEASE_INTERVAL=60  # Proactively release shift keys every 60 seconds
LAST_KEY_CHECK_TIME=0
LAST_PREVENTIVE_RELEASE_TIME=0
...
check_for_stuck_keys() {
    ...
    #                    F1=67 (Deskflow screen switch key, gets stuck)
    ...
    # Auto-detect the physical keyboard: prefer actual keyboard over peripheral devices
    # Try AT Translated first (built-in keyboard), fallback to other keyboards
    physical_id=$(xinput list | grep "slave  keyboard" | grep "AT Translated" | head -1 | grep -oP 'id=\K[0-9]+')

    # If AT Translated not found, try other keyboards excluding peripherals
    if [ -z "$physical_id" ]; then
        physical_id=$(xinput list | grep "slave  keyboard" \
            | grep -v -E "XTEST|Power Button|Video Bus|Sleep Button|Hotkey|HID events|Consumer Control|Audio|Razer|Logitech" \
...
unstick_modifier_keys() {
    ...
    if command -v xdotool >/dev/null 2>&1; then
        # Release all variations of modifier keys + F1 (Deskflow switch key)
        xdotool keyup Shift_L Shift_R Control_L Control_R Alt_L Alt_R Super_L Super_R Caps_Lock F1 2>/dev/null
    ...
    # Method 3: Reset keyboard layout and fix caps lock
...
preventive_release_shift() {
    # Silently release commonly-stuck keys as a preventive measure
    # This catches stuck keys that the detection method misses
    # Includes F1 (Deskflow screen switch hotkey) and Alt (modifier often stuck during switch)
    if command -v xdotool >/dev/null 2>&1; then
        xdotool keyup shift Shift_L Shift_R Alt_L Alt_R F1 2>/dev/null
    fi
}
...
click_reconnect_button() {
    ...
    # Alternative: try keyboard shortcut (based on the source code we saw F5 for restart)
...
start_deskflow_if_needed() {
    if ! is_deskflow_gui_running; then
        ...
        DISPLAY=${DISPLAY:-:0} nohup flatpak run org.deskflow.deskflow > /tmp/deskflow-auto.log 2>&1 &
        sleep 10
...
while true; do
    ...
    # Preventive release of shift keys (silent, runs every 60 seconds)
    if [ $((current_time - LAST_PREVENTIVE_RELEASE_TIME)) -ge $PREVENTIVE_RELEASE_INTERVAL ]; then
        preventive_release_shift
        LAST_PREVENTIVE_RELEASE_TIME=$current_time
    fi
